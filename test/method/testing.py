...
if __name__ == "__main__":
    ...
    if target in ["folder", "file"]:
        os.system(
            "python -m data_collection.extract_functions --source {0} --output {1}".format(source, ".temp_predict"))
        os.system(
            "python -m data_collection.extract_data --source {0}/extracted_functions.json -n 8 --output {1}".format(
    ...
    if model == "codet5":
        ...
        with open(".temp_predict/extracted_condition_message_pairs.json") as srcs:
            data = json.load(srcs)
        parsable_data, vectorized_data = clean_tokenize_vectorize_no_shuffle(data,
                                                                             "models/embedding/embed_if_32.mdl/embed_if_32.mdl",
                                                                             64, 32, 8)
        bilstm = load_model(model_path)
        # bilstm = keras.layers.TFSMLayer(model_path)
        ...
        normalize_p = (tf.reduce_sum(tf.square(anchor_embedding), -1) + tf.reduce_sum(tf.square(positive_embedding),
                                                                                      -1)) / 2
