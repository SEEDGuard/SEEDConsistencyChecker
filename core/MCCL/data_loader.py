...
def get_data_splits(input_path, comment_type_str="Return", ignore_ast=False):
...
def load_cleaned_test_set(comment_type_str='Return'):
    ...
    for comment_type in comment_types:
        resources_path = os.path.join(DEFAULT_DATA_PATH, 'resources', comment_type, 'clean_test_ids.json')
        with open(resources_path) as f:
            test_ids.extend(json.load(f))
    return test_ids

def load_processed_data(input_path, comment_type_str, ignore_ast):
    ...
    for comment_type in comment_types:
        path = os.path.join(input_path, comment_type)
        loaded = load_raw_data_from_path(path)
        category_high_level_details_path = os.path.join(input_path, 'resources', comment_type, 'high_level_details.json')
        ...
        if not ignore_ast:
            ast_path  = os.path.join(input_path, 'resources', comment_type, 'ast_objs.json')
