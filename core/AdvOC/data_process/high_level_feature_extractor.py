...
from utils import subtokenize_token, tokenize_clean_code
...
def extract_arguments(code_block):
    i = 0
    while i < len(code_block):  # 方法前的注释,注解,空行等进行跳过
    ...
    for argument in arguments:
        argument_tokens = re.findall(r"[a-zA-Z0-9_]+|[^\sa-zA-Z0-9_]", argument.strip())
...
def extract_throwable_exceptions(code_block):
    ...
    exception_tokens = [t for t in tokenize_clean_code(throws_string) if not is_operator(t)]
...
def get_method_elements(code_block):
    ...
    for throwable_exception in throwable_exception_tokens:
        throwable_exception_subtokens.extend(subtokenize_token(throwable_exception))
    ...
    for throw_statement in throw_statements:
        throw_statement_tokens.extend([t for t in tokenize_clean_code(throw_statement) if not is_operator(t)])
        throw_statement_subtokens.extend([t for t in subtokenize_token(throw_statement) if not is_operator(t)])
    ...
    for argument_name in argument_names:
        argument_name_tokens.extend([t for t in tokenize_clean_code(argument_name) if not is_operator(t)])
        argument_name_subtokens.extend([t for t in subtokenize_token(argument_name) if not is_operator(t)])

    for argument_type in argument_types:
        argument_type_tokens.extend([t for t in tokenize_clean_code(argument_type) if not is_operator(t)])
        argument_type_subtokens.extend([t for t in subtokenize_token(argument_type) if not is_operator(t)])

    return_statement_tokens = []
    return_statement_subtokens = []
    for return_statement in return_statements:
        return_statement_tokens.extend([t for t in tokenize_clean_code(return_statement) if not is_operator(t)])
        return_statement_subtokens.extend([t for t in subtokenize_token(return_statement) if not is_operator(t)])

    return_type_tokens = [t for t in tokenize_clean_code(return_type) if not is_operator(t)]
    return_type_subtokens = [t for t in subtokenize_token(return_type) if not is_operator(t)]

    method_name = extract_method_name(code_block)
    method_name_tokens = [method_name]
    method_name_subtokens = subtokenize_token(method_name)
