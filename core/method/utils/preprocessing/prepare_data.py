...
def prepare_tr_triplet(tr_data):
    ...
    tr_hard_inconsistent = []
    for t in tr:
        tr_inconsistent.extend(
            [(t[0][0], t[0][1], it[1]) if it[0] == t[0][0] else (t[0][1], t[0][0], it[0]) for it in t[1]])
    for t in tr_hard:
        tr_hard_inconsistent.extend(
            [(t[0][0], t[0][1], it[1]) if it[0] == t[0][0] else (t[0][1], t[0][0], it[0]) for it in t[1]])
    return tr_inconsistent, tr_hard_inconsistent
...
if __name__ == "__main__":
    ...
    if model == "bilstm":
        ...
        for key in data:
            print(key)
            if key in ["condition", "message", "pattern", "embed", "codex_triplet", "random_triplet", "consistent",
                       "inconsistent"]:
                with open(data[key]) as dkl:
                    data_load = json.load(dkl)
            else:
